...
<div class="section-header"><h2>Pod Logs (kubectl)</h2><div class="dl-group"><button class="btn-dl" onclick="downloadCSV('podLogs')" title="Download pod logs summary CSV">&#x2B73; CSV</button></div></div>
<div class="card table-wrap" style="margin-bottom:20px;">
  <table id="pod-logs-table">
    <thead><tr><th>Pod</th><th>Deployment</th><th>Lines</th><th>Errors</th><th>Warnings</th><th>API Reqs</th><th>Health Checks</th><th>HTTP Codes</th></tr></thead>
    <tbody>
      ${podLogSummary.map(p => {
        const httpSummary = Object.entries(p.httpCodes || {}).sort((a,b) => b[1]-a[1]).map(([c,n]) => `${c}:${n}`).join(', ');
        const errClass = p.errorLines > 0 ? 'style="color:var(--red)"' : '';
        return `<tr>
          <td><code>${esc(p.podName.length > 45 ? p.podName.substring(0,45) + '...' : p.podName)}</code></td>
          <td>${esc(p.deployment)}</td>
          <td>${p.totalLines.toLocaleString()}</td>
          <td ${errClass}>${p.errorLines}</td>
          <td>${p.warnLines}</td>
          <td>${p.apiRequests.toLocaleString()}</td>
          <td>${p.healthChecks.toLocaleString()}</td>
          <td style="font-size:0.75rem">${httpSummary || '-'}</td>
        </tr>`;
      }).join('\n')}
    </tbody>
  </table>
  ${podLogSummary.some(p => p.topEndpoints && p.topEndpoints.length > 0) ? `
  <h3 style="margin-top:16px;">Top API Endpoints (across all pods)</h3>
  <table>
    <thead><tr><th>Endpoint</th><th>Total Requests</th></tr></thead>
    <tbody>
      ${(() => {
        const merged = {};
        podLogSummary.forEach(p => (p.topEndpoints || []).forEach(e => { merged[e.endpoint] = (merged[e.endpoint] || 0) + e.count; }));
        return Object.entries(merged).sort((a,b) => b[1]-a[1]).slice(0,15).map(([ep, cnt]) =>
          `<tr><td><code>${esc(ep)}</code></td><td>${cnt.toLocaleString()}</td></tr>`
        ).join('\n');
      })()}
    </tbody>
  </table>` : ''}
...
function downloadCSV(type) {
  ...
  if (type === 'errorTimeline') {
    ...
    csv = 'Pod,Deployment,Total Lines,Errors,Warnings,API Requests,Health Checks,Service\\n';
    (REPORT_DATA.podLogs || []).forEach(function(p) {
      csv += '"' + p.podName + '","' + p.deployment + '",' + p.totalLines + ',' + p.errorLines + ',' + p.warnLines + ',' + p.apiRequests + ',' + p.healthChecks + ',"' + (p.serviceName||'') + '"\\n';
    });
...
function applyFilters() {
  ...
  /* --- 6. Pod Errors (horizontal bar) — recompute from filtered timeline --- */
  if (chartMap.podErrors) {
    // Recompute per-pod counts from filtered timeline categories
    // Since we don't have per-pod per-timestamp data, we scale proportionally
    var totalOriginal = 0;
    REPORT_DATA.categories.forEach(function(c) { totalOriginal += c.count; });
    var totalFiltered = totalFilteredErrors;
    var ratio = totalOriginal > 0 ? totalFiltered / totalOriginal : 0;
    ...
    chartMap.podErrors.data.datasets[0].data = pods.map(function(p) {
      return Math.round(p.count * ratio);
    });
  ...
  /* --- 12. Bot table — recompute from filtered timeline (proportional) --- */
  var botBody = document.getElementById('bot-table-body');
  if (botBody && REPORT_DATA.topBots.length > 0) {
    var html = '';
    REPORT_DATA.topBots.forEach(function(b) {
      var scaledCount = totalOriginal > 0 ? Math.round(b.count * ratio) : b.count;
      var topCat = Object.keys(b.categories).sort(function(a,c){return b.categories[c]-b.categories[a];})[0] || '-';
      html += '<tr><td><code>' + escHtml(b.id) + '</code></td><td>' + numFmt(scaledCount) + '</td><td>' + topCat.replace(/_/g,' ') + '</td><td>' + (b.pods?b.pods.length:0) + '</td></tr>';
    });
...
function setQuickRange(minutes) {
  ...
  var endDate = new Date(bounds.max);
  var startDate = new Date(endDate.getTime() - minutes * 60 * 1000);
  if (startDate.getTime() < bounds.min) startDate = new Date(bounds.min);
