...
async function generateReport(data, outputPath) {
  ...
  // Pod logs summary for dashboard — include time buckets for client-side time filtering
  const podLogSummary = (podLogData && podLogData.pods) ? podLogData.pods.map(p => ({
    ...
    // Time-bucketed data (5-min buckets) for accurate time-filtered counts
    // Each bucket: { t: timestamp, l: lines, e: errors, w: warns, a: apiReqs, h: healthChecks, ep: [{endpoint, count}] }
    tb: (p.timeBucketsArray || [])
  })) : [];
...
function downloadCSV(type) {
  ...
  if (type === 'errorTimeline') {
    ...
    var BUCKET_MS_CSV = 5 * 60 * 1000;
    csv = 'Pod,Deployment,Total Lines,Errors,Warnings,API Requests,Health Checks,Service,First Timestamp,Last Timestamp\\n';
    (REPORT_DATA.podLogs || []).forEach(function(p) {
      var depl = document.getElementById('filter-deployment') ? document.getElementById('filter-deployment').value : 'all';
      if (depl !== 'all' && p.deployment !== depl) return;
      if (!startTs && !endTs) {
        ...
        var buckets = p.tb || [];
        var sL=0,sE=0,sW=0,sA=0,sH=0,any=false;
        for (var bi=0;bi<buckets.length;bi++) {
          var b=buckets[bi], bEnd=b.t+BUCKET_MS_CSV;
          if (endTs && b.t > endTs) continue;
          if (startTs && bEnd < startTs) continue;
          any=true; sL+=b.l; sE+=b.e; sW+=b.w; sA+=b.a; sH+=b.h;
        }
        if (!any && buckets.length > 0) return;
        csv += '"' + p.podName + '","' + p.deployment + '",' + sL + ',' + sE + ',' + sW + ',' + sA + ',' + sH + ',"' + (p.serviceName||'') + '",' + (p.firstTimestamp ? fmtDateTimeFull(p.firstTimestamp) : '') + ',' + (p.lastTimestamp ? fmtDateTimeFull(p.lastTimestamp) : '') + '\\n';
      }
    });
...
function applyFilters() {
  ...
  /* --- 15. Pod Logs table (time-bucketed filtering + deployment) --- */
  var podLogsBody = document.getElementById('pod-logs-body');
  if (podLogsBody && REPORT_DATA.podLogs && REPORT_DATA.podLogs.length > 0) {
    var hasTimeFilter = !!(startTs || endTs);
    var BUCKET_MS = 5 * 60 * 1000;
    var filteredPodStats = [];

    REPORT_DATA.podLogs.forEach(function(p) {
      // Deployment filter
      if (deployment !== 'all' && p.deployment !== deployment) return;

      if (!hasTimeFilter) {
        // No time filter — show full totals
        filteredPodStats.push({
          pod: p, lines: p.totalLines, errors: p.errorLines, warns: p.warnLines,
          apiReqs: p.apiRequests, healthChecks: p.healthChecks, endpoints: p.topEndpoints || []
        });
        return;
      }

      // Time filter active — sum only matching time buckets
      var buckets = p.tb || [];
      if (buckets.length === 0) {
        // No bucket data — fall back to overlap check with full totals
      ...
      var sumLines = 0, sumErrors = 0, sumWarns = 0, sumApiReqs = 0, sumHealth = 0;
      var mergedEp = {};
      var hasAny = false;

      for (var bi = 0; bi < buckets.length; bi++) {
        var b = buckets[bi];
        var bEnd = b.t + BUCKET_MS;
        // Bucket overlaps with filter range?
        if (endTs && b.t > endTs) continue;
        if (startTs && bEnd < startTs) continue;
        hasAny = true;
        sumLines += b.l;
        ...
        sumApiReqs += b.a;
        sumHealth += b.h;
        (b.ep || []).forEach(function(e) {
          mergedEp[e.endpoint] = (mergedEp[e.endpoint] || 0) + e.count;
        });
      }

      if (!hasAny) return; // Pod has no data in the selected time range

      var epArr = Object.keys(mergedEp).map(function(k) { return { endpoint: k, count: mergedEp[k] }; })
        .sort(function(a,b) { return b.count - a.count; }).slice(0, 5);

      filteredPodStats.push({
        pod: p, lines: sumLines, errors: sumErrors, warns: sumWarns,
        apiReqs: sumApiReqs, healthChecks: sumHealth, endpoints: epArr
      });
    });

    var plHtml = '';
    filteredPodStats.forEach(function(ps) {
      var p = ps.pod;
      ...
      var errStyle = ps.errors > 0 ? ' style="color:var(--red)"' : '';
      var timeStr = '';
      if (p.firstTimestamp && p.lastTimestamp) {
      ...
      plHtml += '<tr><td><code>' + escHtml(podLabel) + '</code></td><td>' + escHtml(p.deployment) + '</td><td>' + numFmt(ps.lines) + '</td><td' + errStyle + '>' + ps.errors + '</td><td>' + ps.warns + '</td><td>' + numFmt(ps.apiReqs) + '</td><td>' + numFmt(ps.healthChecks) + '</td><td style="font-size:0.75rem">' + (httpSummary||'-') + '</td><td style="font-size:0.75rem;white-space:nowrap">' + timeStr + '</td></tr>';
    });
    ...
    if (plTitle) plTitle.textContent = 'Pod Logs (kubectl) — ' + filteredPodStats.length + ' of ' + REPORT_DATA.podLogs.length + ' pods';

    // Update Top API Endpoints from time-filtered pod data
    var epBody = document.getElementById('top-endpoints-body');
    if (epBody) {
      var merged = {};
      filteredPodStats.forEach(function(ps) {
        (ps.endpoints || []).forEach(function(e) { merged[e.endpoint] = (merged[e.endpoint] || 0) + e.count; });
      });
      ...
      if (epTitle) epTitle.textContent = 'Top API Endpoints (' + sorted.length + ' endpoints from ' + filteredPodStats.length + ' pods)';
    }
  }
