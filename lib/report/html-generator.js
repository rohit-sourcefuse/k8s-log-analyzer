...
function setQuickRange(minutes) {
  ...
  // Smart anchor: if the range from global max misses all error data,
  // and the requested window is SHORT (≤ 1h), re-anchor to the error data end.
  // For larger ranges (6h+), keep global max as anchor — the wider window
  // should naturally include more data sources even if errors are distant.
  var errBounds = bounds.errors;
  var hasErrors = errBounds.max > 0;
  if (hasErrors && minutes <= 60) {
    var wouldCatchErrors = (startMs <= errBounds.max && endMs >= errBounds.min);
    if (!wouldCatchErrors) {
      // Re-anchor: set end to error data end + 5min buffer
