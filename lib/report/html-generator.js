...
function applyFilters() {
  ...
  /* --- 6b. Pod Errors (horizontal bar) — recompute from filtered timeline --- */
  ...
  /* --- 15. Pod Logs table (filtered by time overlap) --- */
...
function setQuickRange(minutes) {
  ...
  // Smart anchor: Use the end of the data as anchor.
  // But if the requested range is small and would miss all error data,
  // also check if we should anchor to the error data end instead.
  var endMs = bounds.max;
  var startMs = endMs - minutes * 60 * 1000;

  // Check if this range catches any error timeline data
  var errBounds = bounds.errors;
  var hasErrors = errBounds.max > 0;
  if (hasErrors) {
    var wouldCatchErrors = (startMs <= errBounds.max && endMs >= errBounds.min);
    if (!wouldCatchErrors && minutes <= 360) {
      // The requested small range misses ALL error data.
      // Re-anchor to center around the error data end so user sees something useful.
      endMs = Math.min(errBounds.max + 5 * 60 * 1000, bounds.max); // 5 min buffer after last error
      startMs = endMs - minutes * 60 * 1000;
    }
  }
